...
class RoomBookingService {
  ...
  async create(bookingData, user) {
    ...
    const bookingSequence = lastBooking
      ? (lastBooking.bookingSequence || 0) + 1
      : 1;
  ...
  async findMine(username) {
    return await RoomBooking.find({ createdByUsername: username })
      .populate("room", "roomNumber roomType capacity")
      .sort({ createdAt: -1 });
  }
  ...
  async requestCancellation(id, user) {
    ...
    if (booking.bookingStatus !== RoomBookingStatus.BOOKED) {
      throw new AppError(
        "Only BOOKED reservations can request cancellation",
        400,
      );
    }
  ...
  async approveCancellation(id) {
    ...
    if (booking.bookingStatus !== RoomBookingStatus.CANCELLATION_REQUESTED) {
      throw new AppError(
        "Booking does not have a pending cancellation request",
        400,
      );
    }
